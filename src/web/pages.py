"""Streamlit pages and interaction flow for Chess Improvement Coach."""
...
from src.models import AnalyzedMove, MoveQuality, UserLevel
from src.move_classifier import MoveClassifier
from src.report import GameReportBuilder
from src.services.practice_service import PracticeMoveError
...
from src.web.resources import WebResources, create_resources
...
def interactive_board_css(
    *,
    ...
    row_selectors = ",".join(
        f".st-key-{key_prefix}_rank_{rank + 1}"
        for rank in range(8)
        if rank != (0 if flipped else 7)
    )
    layout = (
        f"""
...
def render_interactive_board(
    ...
    with st.container(key=f"{key_prefix}_interactive_chess_board"):
        for rank in ranks:
            with st.container(key=f"{key_prefix}_rank_{rank + 1}"):
...
def render_dashboard(resources: WebResources) -> None:
    ...
    if summary.success_rate_change is None:
        st.caption(
            "20 attempts are required to compare the latest 10 with the previous 10."
        )
    else:
        st.metric(
...
def render_practice(resources: WebResources) -> None:
    ...
    st.caption(
        "Select a piece and its destination. Incorrect legal moves receive fresh coaching feedback."
    )
    ...
    with right:
        ...
        if result is not None:
            ...
            st.caption(
                f"Next review: {result.updated_position.next_review_at:%Y-%m-%d %H:%M UTC}"
            )
...
def update_position_evaluation(resources: WebResources) -> None:
    ...
    with st.spinner("Updating evaluation..."):
        st.session_state.web_position_evaluation = resources.analysis_engine.analyze(
            game.board, depth=8
        )
...
def submit_web_move(resources: WebResources, move_text: str) -> None:
    ...
    try:
        ...
        analysis = MoveAnalyzer(resources.analysis_engine).analyze_move(
            game.board, move, depth=12
        )
    ...
    detection = (
        ...
        in {MoveQuality.INACCURACY, MoveQuality.MISTAKE, MoveQuality.BLUNDER}
        else None
    )
...
def render_play(resources: WebResources) -> None:
    ...
    if "web_game" not in st.session_state:
        ...
        with st.form("new_game"):
            ...
            elo = st.slider(
                "Opponent Elo", minimum, maximum, min(1600, maximum), step=10
            )
